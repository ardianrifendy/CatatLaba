...
import { ArrowLeft, Fingerprint, Check, Unlock, Lock, Grid, KeyRound, Delete, Shield, HelpCircle } from 'lucide-react'
...
export function SecuritySubScreen({ onBack }: { onBack: () => void }) {
  ...
  const savedQuestion = useSecurityStore((s) => s.recoveryQuestion)
  ...
  // Passcode Setup Flow State (1: Input, 2: Confirm, 3: Recovery Question)
  ...
  // Recovery Option Setup State
  const [recoveryQuestion, setRecoveryQuestion] = useState(
    savedQuestion || DEFAULT_RECOVERY_QUESTIONS[0]!
  )
    ...
    if (nextPin.length === pinLength) {
      if (setupStep === 1) {
        ...
        if (nextPin === firstPasscode) {
          setSetupStep(3)
          setCurrentPinInput('')
          toast.success('PIN cocok! Sekarang atur Pertanyaan Pemulihan.')
        } else {
          triggerShake()
  ...
  function handlePatternTouchEnd() {
    if (!isDrawingPattern) return
    ...
    if (setupStep === 1) {
      ...
      if (patternStr === firstPasscode) {
        setSetupStep(3)
        setPatternDots([])
        toast.success('Pola cocok! Sekarang atur Pertanyaan Pemulihan.')
      } else {
        triggerShake()
  ...
  function handlePasswordStepSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!passwordInput) {
    ...
    setFirstPasscode(passwordInput)
    setSetupStep(3)
    toast.success('Sandi cocok! Sekarang atur Pertanyaan Pemulihan.')
  }

  // --- FINALIZE LOCK & RECOVERY SETUP ---
  async function handleFinalizeSecuritySetup(e: React.FormEvent) {
    e.preventDefault()
    if (!recoveryAnswer.trim()) {
      toast.error('Jawaban pemulihan wajib diisi.')
      return
    }
    await setLockConfig(
      selectedType,
    ...
    toast.success('Keamanan & Opsi Pemulihan berhasil disimpan!')
  }
  ...
  return (
    <section className="flex flex-col gap-5 ios-page-enter">
      ...
      {/* Biometric & Recovery Settings Card */}
      <div className="flex flex-col gap-2">
        <span className="px-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
          Autentikasi & Pemulihan
        </span>
        <GlassCard className="divide-y divide-glass-border/60 overflow-hidden">
          ...
          {currentLockType !== 'none' && (
            <div className="p-4 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3 min-w-0 flex-1">
                <div className="flex size-10 items-center justify-center rounded-xl bg-amber-500/15 text-amber-500 border border-amber-500/20 shrink-0">
                  <HelpCircle className="size-5" />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-semibold text-foreground truncate">Opsi Pemulihan Kunci</p>
                  <p className="text-xs text-muted-foreground truncate">
                    {savedQuestion || 'Belum diatur'}
                  </p>
                </div>
              </div>
              <GlassButton
                ...
                onClick={() => {
                  setSetupStep(3)
                  setFirstPasscode('')
                  setSelectedType(currentLockType)
                  setShowSetupModal(true)
                }}
      ...
      {showSetupModal &&
        createPortal(
          <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 backdrop-blur-2xl p-4 sheet-overlay" data-state="open">
            <div className="w-full max-w-[330px] p-6 flex flex-col items-center justify-center text-center gap-5 rounded-[32px] border border-glass-border/80 bg-background text-foreground shadow-2xl backdrop-blur-3xl dark:bg-card/95 border-white/20 ios-modal-pop">
              ...
              <div className="flex flex-col items-center gap-2">
                <div className="flex size-14 items-center justify-center rounded-2xl bg-accent/20 text-accent border border-accent/30 shadow-md shadow-accent/20 animate-pulse">
                  {setupStep === 3 ? <HelpCircle className="size-7 text-accent" /> : <Shield className="size-7 text-accent" />}
                </div>
                <h3 className="text-base font-extrabold text-foreground mt-1">
                  {setupStep === 3
                    ? 'Pertanyaan Pemulihan'
                    : selectedType === 'pin'
                      ? (setupStep === 1 ? `Buat PIN ${pinLength}-Digit` : 'Konfirmasi PIN')
                      : selectedType === 'pattern'
                        ? (setupStep === 1 ? 'Gambar Pola Kunci' : 'Konfirmasi Pola')
                        : 'Buat Sandi Teks'}
                </h3>
                <p className="text-xs text-muted-foreground max-w-[260px] leading-relaxed">
                  {setupStep === 3
                    ? 'Gunakan pertanyaan ini untuk mereset pengunci jika lupa passcode'
                    : selectedType === 'pin'
                      ? (setupStep === 1 ? `Masukkan ${pinLength} angka rahasia Anda` : 'Masukkan ulang PIN untuk konfirmasi')
                      : selectedType === 'pattern'
                        ? (setupStep === 1 ? 'Hubungkan minimal 4 titik' : 'Gambar ulang pola untuk konfirmasi')
                        : 'Masukkan kata sandi alfanumerik'}
                </p>
              </div>

              {/* STEP 3: RECOVERY QUESTION & ANSWER SETUP */}
              {setupStep === 3 ? (
                <form onSubmit={handleFinalizeSecuritySetup} className="flex flex-col gap-3.5 w-full text-left">
                  ...
                  <GlassField label="Jawaban Rahasia Pemulihan">
                    <GlassInput
                      type="text"
                      placeholder="Jawaban Anda (bebas kapital)..."
                  ...
                  <GlassButton type="submit" variant="primary" className="h-11 mt-1 font-bold">
                    Simpan Keamanan
                  </GlassButton>
                </form>
              ) : (
                <>
                  ...
                  {selectedType === 'password' && (
                    <form onSubmit={handlePasswordStepSubmit} className={`flex flex-col gap-3 w-full text-left ${isShaking ? 'ios-shake' : ''}`}>
